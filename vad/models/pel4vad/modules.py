"""
models/pel4vad/modules.py
Source: yujiangpu20/PEL4VAD  (unchanged)
"""
import torch
import torch.nn as nn
import torch.nn.functional as F

from models.pel4vad.layers import TCA, DistanceAdj


class XEncoder(nn.Module):
    def __init__(self, d_model, hid_dim, out_dim, n_heads,
                 win_size, dropout, gamma, bias, norm=None):
    ...
    def get_mask(self, window_size, temporal_scale, seq_len):
        ...
        for j in range(temporal_scale):
            for k in range(window_size):
                idx = min(max(j - window_size // 2 + k, 0), temporal_scale - 1)
                m[j, idx] = 1.
