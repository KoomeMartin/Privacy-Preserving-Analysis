...
Source: https://github.com/v-iashin/video_features/models/i3d/i3d_src/i3d_net.py
Based on: Quo Vadis, Action Recognition? A New Model and the Kinetics Dataset (https://arxiv.org/abs/1705.07750)
"""
...
class MaxPool3dTFPadding(nn.Module):
    def __init__(self, kernel_size, stride, padding='SAME'):
        ...
        if padding == 'SAME':
            padding_shape = get_padding_shape(kernel_size, stride)
            self.simplify_pad, self.pad_size = simplify_padding(padding_shape)
        else:
            self.simplify_pad = True
            self.pad_size = 0

        self.maxpool3d = nn.MaxPool3d(kernel_size=kernel_size, stride=stride, padding=self.pad_size if (padding == 'SAME' and self.simplify_pad) else 0)
        if padding == 'SAME':
            if not self.simplify_pad:
                self.pad = nn.ConstantPad3d(padding_shape, float('-inf'))

    def forward(self, inp):
        if self.padding == 'SAME' and not self.simplify_pad:
            inp = self.pad(inp)
            return self.maxpool3d(inp)
        else:
            return self.maxpool3d(inp)
